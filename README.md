...
<p align="center">
  ...
  <img alt="Тестов 81" src="https://img.shields.io/badge/%D1%82%D0%B5%D1%81%D1%82%D0%BE%D0%B2-81-brightgreen">
...
минуту, пока идёт пара или вот-вот начнётся, иначе на момент, когда картинка
действительно изменится. Будильник неточный (`setAndAllowWhileIdle`): точные
...
81 тест гоняется **на настоящих страницах** сайта (осенний семестр
...
Сам сторож покрыт тестами без сети:
...
**Tests.** `./gradlew test` runs 81 JVM tests against real pages saved from
